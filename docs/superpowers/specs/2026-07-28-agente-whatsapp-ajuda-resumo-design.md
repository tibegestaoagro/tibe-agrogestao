...
tenant tem mais de uma, e como a confirmação de venda de animal funciona), `ajuda` e `resumo` reusam exatamente esse mecanismo, só com prompts novos.
...
`INTENT_ACCESS`: `{ module: null, action: "read" }` (sem perfil obrigatório, a checagem de perfil acontece por tópico, dentro do handler).
