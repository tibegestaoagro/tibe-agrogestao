...
export async function GET(request: Request) {
  ...
  try {
    ...
    // É bookkeeping para o painel da plataforma: o acesso em si é decidido
    // por `getBillingAccess()` a cada request, então falhar aqui não libera
    // nem bloqueia ninguém indevidamente.
    const swept = await sweepCanceledSubscriptions().catch(() => ({
      archived: 0,
      unarchived: 0,
      pending_decision: 0,
    }));
