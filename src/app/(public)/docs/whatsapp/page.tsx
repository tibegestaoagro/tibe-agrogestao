  ...
  return (
    <article className="max-w-3xl space-y-10 text-sm leading-relaxed text-texto-secundario">
      <div>
        ...
        <div className="mt-4 rounded-lg bg-atencao-suave p-4 text-sm text-atencao-tinta">
      ...
      <section>
        <h2 className="text-lg font-semibold text-texto">Fluxo completo</h2>
...
8. Se a ação exigia confirmação, o próximo "sim"/"não" do usuário volta ao passo 4,
   e o N8N deve reenviar a MESMA intenção+parâmetros com confirmed: true/false`}
      ...
      <section>
        <h2 className="text-lg font-semibold text-texto">Identificação de contato</h2>
        <p className="mt-2">
          <code className="rounded bg-superficie-afundada px-1">resolve-contact</code> é o único endpoint do sistema que
          legitimamente busca através de todos os tenants: ele ainda não sabe a quem o telefone pertence. Ele
          busca primeiro um <code className="rounded bg-superficie-afundada px-1">WhatsAppContact</code> já vinculado; se não
          existir, busca um <code className="rounded bg-superficie-afundada px-1">User</code> ativo com aquele telefone
          cadastrado em algum tenant e cria o vínculo na hora (esse é o <em>first_contact</em>). Se nenhum User
          tiver aquele telefone, devolve <code className="rounded bg-superficie-afundada px-1">identified: false</code>: o
          número simplesmente não pertence a ninguém no sistema, e nenhuma ação de escrita pode ser executada.
        </p>
      </section>

      <section>
        <h2 className="text-lg font-semibold text-texto">Intenções suportadas</h2>
        <table className="mt-3 w-full border-collapse text-left text-xs">
          <thead>
            <tr className="border-b border-borda text-texto-discreto">
          ...
          <tbody className="divide-y divide-borda">
            {INTENTS.map(([intent, params, perm]) => (
              <tr key={intent}>
                ...
                <td className="py-2 pr-3 text-texto-secundario">{params}</td>
                <td className="py-2 text-texto-secundario">{perm}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <p className="mt-3">
          Cada intenção roteia para a mesma função de <code className="rounded bg-superficie-afundada px-1">lib/actions/*</code> usada
      ...
      <section>
        <h2 className="text-lg font-semibold text-texto">Confirmação para ações de alto valor</h2>
        <p className="mt-2">
          ...
          <code className="rounded bg-superficie-afundada px-1">requires_confirmation: true</code> junto com os parâmetros
          originais em <code className="rounded bg-superficie-afundada px-1">auxiliary_data</code>. O N8N deve reenviar a
          mesma intenção com <code className="rounded bg-superficie-afundada px-1">confirmed: true</code> quando o usuário
          concordar.
        </p>
        <p className="mt-2">
          Como camada de segurança independente do LLM, o Tibé também interpreta “sim”/“não” a partir do texto
          bruto da mensagem (campo <code className="rounded bg-superficie-afundada px-1">message_text</code>), reconhecendo
      ...
      <section>
        <h2 className="text-lg font-semibold text-texto">Contexto de curto prazo</h2>
        <p className="mt-2">
          Toda mensagem recebida e toda resposta enviada são gravadas em{" "}
          <code className="rounded bg-superficie-afundada px-1">AgentConversationLog</code>. As últimas 5 interações de cada
          contato voltam em <code className="rounded bg-superficie-afundada px-1">meta.recent_history</code> na resposta de{" "}
          <code className="rounded bg-superficie-afundada px-1">resolve-contact</code>: é esse histórico que o N8N deve
          enviar ao LLM para manter continuidade de conversa (ex: entender “e o segundo?” depois de uma lista).
        </p>
      </section>

      <section>
        <h2 className="text-lg font-semibold text-texto">Relatórios pelo WhatsApp</h2>
        <p className="mt-2">
          <code className="rounded bg-superficie-afundada px-1">gerar_relatorio</code> com{" "}
          <code className="rounded bg-superficie-afundada px-1">tipo: &quot;financeiro&quot;</code> devolve um link assinado de PDF
          real (mesma função usada pelo botão “Exportar” do painel web). Os tipos{" "}
          <code className="rounded bg-superficie-afundada px-1">rebanho</code>,{" "}
          <code className="rounded bg-superficie-afundada px-1">lavoura</code> e{" "}
          <code className="rounded bg-superficie-afundada px-1">prestador</code> ainda respondem “não disponível”: não há
